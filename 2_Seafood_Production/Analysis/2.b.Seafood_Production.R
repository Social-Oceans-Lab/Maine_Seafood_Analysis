...
lines_label <- production_sum %>%
  ...
  bind_cols(posyend = seq(6e6, 4e8, 4e8 / 7))                  #location of the right end of the lines going from graph to labels          
...
axis_p1 <- tibble(x = c(2011, 2015, 2017),                  #years less than the years the values are associated with so they can bee seen. 
                  xend = c(2012, 2016, 2018),               #years I'm showing in white near the peaks in the graph
                  y = c(3.15e8, 2.95e8, 2.75e8),              #locations for those numbers
                  yend = c(3.15e8, 2.95e8, 2.75e8),           #ditto (I think?)  
                  yend_lab = c("315 M", "295 M", "275 M"))  #I created this so we could have simple labels - call this as the "label"    
...
production_species_plt <- production_sum %>%
  ...
  geom_text(data = axis_p1, aes(x = x, y = yend, label = yend_lab), 
            color = "white", hjust = 0, nudge_y = 1e7, size = 3, family = "roboto", inherit.aes = FALSE) +           #labels on peaks  
  geom_text(data = year_list, aes(x = year, y = -9e6, label = year), 
            color = "white", hjust = 1.25, vjust = 0.5, angle = 45,  
            size = 3.5, family = "roboto", inherit.aes = TRUE) +                                                     #new x-axis       
  geom_area(aes(Year, total, fill = NOAA.category), color = "grey1") +                                              #bar graph?
  ...
  geom_label(data = lines_label, aes(x = 2025, y = posyend, label = NOAA.category, color = NOAA.category), 
             fill = bck_color, size = 3.5, family = "khula", nudge_y = 1e7, label.size = 0, 
             hjust = 0, fontface = "bold", inherit.aes = FALSE) +                                                    #spp names near lines
  geom_label(data = lines_label, aes(x = 2025, y = posyend, label = glue("{comma(total)} Pounds"), color = NOAA.category), 
             fill = bck_color, size = 3.5, family = "khula", nudge_y = -1e7, label.size = 0, 
             hjust = 0, fontface = "bold", inherit.aes = FALSE) +                                                    #values near lines
  annotate("text", x = 2010, y = 4.25e8, label = "(A) Pounds by Species Category (2010-2020)", 
           family = "khula", size = 5, color = "white", hjust = 0) +                                                 #label
  annotate("text", x = 2033, y = 4.25e8, label = "(B) Total Pounds Landed by Region (2020)", 
           family = "khula", size = 5, color = "white", hjust = 0) +                                                 #label
...
axis_p2 <- tibble(x = c(1, 2, 3, 4, 5, 6, 7),                          #size of x-axis
                  ...
                  y = seq(0e8, 1.25e8, 2e7),                              #y-axis
                  yend = seq(0e8, 1.25e8, 2e7),                           #end of y-axis
                  yend_lab = c("0", "20 M", "40 M", 
                               "60 M", "80 M", "100 M", "120 M"))          #simple labels
...
final <- production_species_plt +                                                                     #line graph 
  inset_element(production_cty_plt, 0.555, -0.01, 1, 0.94) +                                          #bar graph and location coords
  inset_element(legend_plt, 0.9, 0.1, 0.98, 0.4) +                                                    #legend and location coords
  plot_annotation(
    caption = "Data Source: ACCSP 2021",
    title = "Seafood Harvest in Maine by Volume (2010-2020)",
    theme = theme(
...
ragg::agg_png(here::here("render", paste0("2.b.Total_Seafood_Production_ME_2010_2020", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png")), 
...
ggsave("2.b.Total_Seafood_Production_ME_2010_2020.pdf", width = 15, height = 9)
